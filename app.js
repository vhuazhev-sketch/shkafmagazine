...
(function () {
  ...
  function renderGrid() {
    ...
    // обновляем список всех отзывов при отрисовке каталога
    renderAllReviews();
  }
  ...
  function renderAdminActions() {
    ...
    deletePhotoBtn.addEventListener('click', () => {
      ...
      // Удаляем данные изображения и варианты, которые содержат dataURI
      delete currentProduct.imageData;
      // Оставляем только эмодзи и ссылки, удаляем data URI варианты
      currentProduct.variants = currentProduct.variants.filter(v => !(v.startsWith('data:')));
      if (currentProduct.variants.length === 0) {
        // если все варианты были изображениями, возвращаем эмодзи как единственный вариант
        currentProduct.variants = [currentProduct.emoji || '❓'];
      }
      saveProducts();
      renderGrid();
      renderProduct();
  ...
  function renderAllReviews() {
    ...
    // Создаём список всех отзывов с привязкой к товару
    products.forEach(prod => {
      prod.reviews.forEach(r => {
        const card = document.createElement('div');
        card.className = 'review-card';
        // определяем иконку товара
        let iconHTML;
        if (prod.imageData) {
          iconHTML = `<img src="${prod.imageData}" style="width:20px;height:20px;border-radius:4px;margin-right:6px;vertical-align:middle;">`;
        } else {
          iconHTML = `<span style="margin-right:6px;">${prod.emoji}</span>`;
        }
        card.innerHTML = `
          <div><strong>${iconHTML}${prod.name}</strong> — <em>${r.author}</em></div>
          <div>${r.text}</div>
        `;
